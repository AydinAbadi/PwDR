...
{\small{
\begin{center}
\begin{mybox}[colback=white,  width=81mm, height=48mm, left=.3mm, drop fuzzy shadow southwest]{{{$\mathsf{Exp}_{\st 1}^{\st\mathcal{A}}(1^{\st\lambda}\text{, }  
{aux})$}}}
%
...
%\Bigg(
%
  \hs\Big(\underbrace{(m^{\st\mathcal{(B)}}_{\st1}=warning) \wedge (\sum\limits_{\st j=1}^{\st n}w_{\st 1, j}\geq {e})}_{\st(\text{i})}\Big) 
  %
    \vee\\ \hs \Big(\underbrace{(\sum\limits_{\st j=1}^{\st n}w_{\st 1,j}<{e}) \wedge ({v}_{\st 1} =1)}_{\st (\text{ii})}\Big)
 % \Bigg)
  
  %
\vee\\
  %\Bigg(
  \hs  \Big(\underbrace{(\mathtt{checkWarning}(m^{\st\mathcal{(B)}}_{\st1})= 1) \wedge\ \hs (\sum\limits_{\st j=1}^{\st n}w_{\st 2,j}\geq {e})}_{\st (\text{iii})}\Big)
  %
  \vee\\\hs \Big(\underbrace{(\sum\limits_{\st j=1}^{\st n}w_{\st 2,j}< {e}) \wedge ({v}_{\st 2}=1)}_{\st (\text{iv})}\Big)
  %\Bigg)
  
  %
  \vee \\
  %\Bigg(
\hs  \Big(\underbrace{u\notin Q \wedge\mathtt{Sig.ver}( pk,  u, sig) =1}_{\st (\text{v})}\Big) 
  %
   \vee\\\hs \Big(\underbrace{(\sum\limits_{\st j=1}^{\st n}w_{\st 3,j}< {e}) \wedge ( {v}_{\st 3}=1)}_{\st (\text{vi})}\Big)
  %
  %\Bigg)
  %
%\text{s.t.}\\
...
{\small{
\begin{center}
\begin{mybox}[colback=white,  width=80mm, height=51mm, left=-1mm, drop fuzzy shadow southwest]{$\mathsf{Exp}_{\st 2}^{\st\mathcal{A}}(1^{\st\lambda}\text{, }  {aux})$}
%\fbox{\begin{minipage}{3.8 in}
\vspace{-.9mm}
$$
  \begin{array}{l}
...
\Big(\underbrace{(\sum\limits_{\st j=1}^{\st n}w_{\st 1,j}\geq e) \wedge ( v_{\st 1}=0)}_{\st (\text{i})}\Big) 
 %
 
\ \ \vee\\ \Big(\underbrace{( \sum\limits_{\st j=1}^{\st n}w_{\st 2,j}\geq e) \wedge ( v_{\st 2}=0)}_{\st(\text{ii})}\Big) 
 %
\ \ \vee \\ \Big(\underbrace{( \sum\limits_{\st j=1}^{\st n}w_{\st 3,j}\geq e) \wedge ( v_{\st 3}=0)}_{\st(\text{iii})}\Big) 
 %
\ \ \vee \\  \Big(\underbrace{( \sum\limits_{\st j=1}^{\st n}w_{\st 4,j}\geq e) \wedge ( v_{\st 4}=0)}_{\st(\text{iv})}\Big) \\
 %
\end{array} :
    \begin{array}{l}
...
A $\mathsf{pwdr}$ scheme is privacy-preserving if it protects the privacy of (1)  customers, bank, and auditors' sensitive messages from the scheme's non-participants and (2) each auditor's verdict from $\mathcal{DR}$.%  which sees the final verdict. 
