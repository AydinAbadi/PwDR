...
%
$\mathcal{D}_{\st n}$ computes each element of $W$ regardless of what a specific auditor votes; also, it can generate each   $\alpha_{\st i}$ independently, as it knows the single key (of the pseudorandom function) that was used by other auditors to generate these values.  
 %
...
% \end{equation*}
 %
 For instance, when $n=10$ and  $e=6$, then $|W|=386$. Appendix \ref{sec:: Further-Discussion-on-the-Encoding-decoding-Protocol} provides more discussions on the above protocols and their difference from existing voting protocols. 
...
\begin{center}
    \begin{tcolorbox}[enhanced,width=81mm, height=61mm, left=0mm,
...
\setlength{\fboxsep}{1pt}
\begin{center}
    \begin{tcolorbox}[enhanced,width=81mm, height=50mm, left=0mm,
    drop fuzzy shadow southwest,
    colframe=black,colback=white]
   { \small{
    \vspace{-1.3mm}
