...
%
Note that $\mathcal{D}_{\st n}$ computes each element of $W$ regardless of what specific auditor votes; moreover, it can generate each   $\alpha_{\st i}$ independently (without interacting with other parties), as it knows the single key (of the pseudorandom function) that was used by the parties to generate these values.  
