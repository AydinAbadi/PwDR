...
In this section, we outline a formal definition of the payment with dispute resolution (pwdr) notion. Below, we first provide the scheme's syntax.  Then, we formally define its correctness and security properties. 
...
\begin{center}
%{\small{
\begin{mybox}[colback=white,  width=120mm, height=60mm, drop fuzzy shadow southwest]{$\mathsf{Exp}_{\st 1}^{\st\mathcal{A}}(1^{\st\lambda}\text{, }  {aux})$}
...
Note, in the above experiment, we did not explicitly pass payment detail $f$ and auxiliary information ($in_{\st f}, {aux}_{\st f}$) on to the adversary, to let the adversary pick them (on the client's behalf). 
