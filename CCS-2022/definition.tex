...
{\small{
\begin{center}
\begin{mybox}[colback=white,  width=82mm, height=44mm, left=.3mm, drop fuzzy shadow southwest]{{{$\mathsf{Exp}_{\st 1}^{\st\mathcal{A}}(1^{\st\lambda}\text{, }  
{aux})$}}}
%
...
%
 \forall j, j\in [n]:
\Big(\mathtt{verComplaint}( \hat z, \hat{\ddot{\pi}}, g, \hat{\bm{m}}, \hat{\bm{l}}, j, sk_{\st\mathcal D}, aux, pp)\rightarrow \hat{\bm{w}}_{\st j}%= [\hat w_{\st 1,j}, \hat w_{\st 2, j}, \hat w_{\st 3, j}, \hat w_{\st 4, j}]
\Big) \\
...
{\small{
\begin{center}
\begin{mybox}[colback=white,  width=83mm, height=49mm, left=-1mm, drop fuzzy shadow southwest]{$\mathsf{Exp}_{\st 2}^{\st\mathcal{A}}(1^{\st\lambda}\text{, }  {aux})$}
%\fbox{\begin{minipage}{3.8 in}
%\vspace{-.6mm}
$$
  \begin{array}{l}
