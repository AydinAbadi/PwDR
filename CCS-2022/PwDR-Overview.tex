...
\subsection{Challenge 4: Preserving Privacy}
 ...
 Although the use of a public logging mechanism is vital in resolving disputes transparently, if it does not use a  privacy-preserving mechanism, parties' privacy would be violated. To protect the privacy of the bank's and customers' messages from the public, our scheme lets them provably agree on encoding-decoding tokens with which they can encode their messages. Later, either party can provide the token to a third party  (e.g., $\mathcal{D}_{\st i}$) which checks the token's correctness, and decodes the messages. To protect the privacy of the committee members' verdicts from $\mathcal{DR}$, the scheme ensures that  $\mathcal{DR}$  learns only the final verdict without being able to link a verdict to a specific auditor or even learn the number of yes/$1$ and no/$0$ votes. To this end, we develop and use novel threshold voting protocols. 
