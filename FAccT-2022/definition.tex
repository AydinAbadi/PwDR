...
\vspace{-2.5mm}
Informally, a $\mathsf{pwdr}$  has two  properties, namely, \emph{correctness} and \emph{security}. Correctness requires that  the payment journey is completed (in the absence of an adversary) without the need for (i) the honest customer to complain and (ii)  the honest bank to reimburse.  A $\mathsf{pwdr}$  is  secure if it meets three main properties,  (a) security against a malicious victim, (b) security against a malicious bank, and (c) privacy. 
%
